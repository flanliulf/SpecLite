#!/usr/bin/env python3
"""
Resolve customization for a SpecLite skill using three-layer TOML merge.

Reads customization from three layers (highest priority first):
    1. {project-root}/_speclite/custom/{name}.user.toml  (personal, gitignored)
    2. {project-root}/_speclite/custom/{name}.toml       (team/org, committed)
...
no virtualenv — plain `python3` is sufficient.
  ...
  python3 resolve_customization.py --skill /abs/path/to/skill-dir
    ...
    python3 resolve_customization.py --skill ... --key workflow.on_complete
...
def find_project_root(start: Path):
    ...
    while True:
        if (current / "_speclite").exists() or (current / ".git").exists():
            return current
...
def main():
    parser = argparse.ArgumentParser(
        description="Resolve customization for a SpecLite skill using three-layer TOML merge.",
        add_help=True,
    )
    parser.add_argument(

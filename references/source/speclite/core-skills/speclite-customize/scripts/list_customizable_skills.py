...
# ///
"""Enumerate customizable SpecLite skills installed alongside this one.
...
`{project-root}/_speclite/custom/`.

Skills in SpecLite are loaded either from a project-local location (e.g. the
...
def default_skills_root() -> Path:
    ...
    Layout assumption: {skills_root}/speclite-customize/scripts/list_customizable_skills.py.
...
def scan_skills(
    ...
    custom_dir = project_root / "_speclite" / "custom"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "List customizable SpecLite skills installed alongside this one, "
            "grouped by surface (agent vs workflow), with override status "
            "looked up against {project-root}/_speclite/custom/."
        )
    )
    parser.add_argument(
        "--project-root",
        required=True,
        help="Absolute path to the project root (the folder containing _speclite/).",
    )
    parser.add_argument(
        "--skills-root",
        default=None,

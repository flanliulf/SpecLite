...
- a pre-existing team override in _speclite/custom/
...
class ScannerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.custom = self.root / "_speclite" / "custom"
    ...
    def test_agent_only_skill_detected(self):
        ...
        self.assertEqual(entry["name"], "speclite-agent-pm")
    ...
    def test_dual_surface_skill_emits_two_entries(self):
        ...
        self.assertEqual(len(result["agents"]), 1)
        self.assertEqual(len(result["workflows"]), 1)
        self.assertEqual(result["agents"][0]["name"], "speclite-dual")
        self.assertEqual(result["workflows"][0]["name"], "speclite-dual")

    def test_skill_without_customize_toml_ignored(self):
        (self.skills / "speclite-plain").mkdir()
        (self.skills / "speclite-plain" / "SKILL.md").write_text("# plain\n")
        result = MODULE.scan_skills([self.skills], self.root)
    ...
    def test_existing_team_override_flagged(self):
        ...
        (self.custom / "speclite-agent-pm.toml").write_text("[agent]\n")
        result = MODULE.scan_skills([self.skills], self.root)
        entry = result["agents"][0]
    ...
    def test_missing_surface_block_reports_error(self):
        _make_skill(self.skills, "speclite-broken", "[not_a_surface]\nfoo = 1\n")
        result = MODULE.scan_skills([self.skills], self.root)
        self.assertEqual(len(result["agents"]) + len(result["workflows"]), 0)
    ...
    def test_malformed_toml_reports_error_without_aborting(self):
        skill_dir = self.skills / "speclite-bad"
        skill_dir.mkdir()
        ...
        result = MODULE.scan_skills([self.skills], self.root)
        self.assertEqual(len(result["agents"]), 1)
        self.assertEqual(result["agents"][0]["name"], "speclite-good")
    ...
    def test_multiple_skills_roots_are_merged(self):
        ...
        self.assertEqual(names, {"speclite-agent-pm", "speclite-agent-dev"})

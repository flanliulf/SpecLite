...
[技能说明]
    Edit PRD 是 Speclite 规划与产品设计工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-edit-prd`。
    ...
    - 输出文档末尾必须追加 `*本文档由 speclite-edit-prd Skill 自动生成*` 标注。

...
[技能说明]
    Sprint Planning 是 Speclite 实施与交付推进工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Generate sprint status tracking from epics. Use when the user says "run sprint planning" or "generate sprint plan"
...
[注意事项]
    ...
    - 模板或示例骨架位于 `assets/sprint-status-template.yaml`。

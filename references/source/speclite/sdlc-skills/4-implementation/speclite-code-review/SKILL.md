---
name: speclite-code-review
description: "Code Review workflow for Speclite 实施与交付推进, migrated from a legacy source skill with runtime-model conversion. Use when user mentions 'code review', 'speclite code review', 'create code review', 'run code review', 'Code Review', '创建Code Review', '生成Code Review', '执行Code Review', '检查Code Review', '运行Code Review'. Capable of config-driven activation, three-tier customization, source artifact discovery, step-file orchestration, output generation, and completion handoff."
...
[技能说明]
    Code Review 是 Speclite 实施与交付推进工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Review code changes adversarially using parallel review layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) with structured triage into actionable categories. Use when the user says "run code review" or "review this code"
...
[执行流程]
    1. 先完整阅读 `references/workflow-details.md`；该文件是从源入口转换后的权威工作流规约。随后按需读取 `references/workflow-details.md`、`references/steps/step-01-gather-context.md`、`references/steps/step-02-review.md`、`references/steps/step-03-triage.md`、`references/steps/step-04-present.md`。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-code-review`。
    ...
    - 输出文档末尾必须追加 `*本文档由 speclite-code-review Skill 自动生成*` 标注。

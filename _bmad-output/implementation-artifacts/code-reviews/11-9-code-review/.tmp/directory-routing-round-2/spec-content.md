...
- 固定 ownership marker `.tmp/cr-directory-ownership.json` 只证明 pre-summary run 的目录归属；不证明 approval、tracker、gate、scope/hash、freshness、round 或 completion。
...
- `reviewSeries=directory-routing` 的 round 2 必须审完整 current 42 个声明输入；不能只审 CR03 的 8 文件 delta。
- 旧 `evidence-v2` 的 PASS、风险接受或止损授权仅属历史，不能作为本轮通过依据。
- 项目 v2 shared contract 优先于 global legacy `cr-config`；禁止低 quorum fallback、旧命名或清理旧/当前 `.tmp`。

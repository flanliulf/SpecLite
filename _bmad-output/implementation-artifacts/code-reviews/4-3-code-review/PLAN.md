...
针对 Story `4-3` 严格串行完成开发、代码审查、评估、修复循环、规则提炼、TODO 跟踪、收尾和后续本地提交。

## 串行步骤

1. 使用全新的 sub agent 执行 `/bmad-dev-story story 4-3`，模型为 `GPT-5.5`。
2. 使用全新的 sub agent 执行 `/bmenhance-cr-01-reviewer 4-3`，模型为 `GPT-5.5`。
3. 使用全新的 sub agent 执行 `/bmenhance-cr-02-evaluator 4-3`，模型为 `GPT-5.5`。
4. 使用全新的 sub agent 执行 `/bmenhance-cr-03-fixer 4-3`，模型为 `GPT-5.5`。

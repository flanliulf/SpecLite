...
- Counts：raw 269 / entries 269 / unmapped 0（validator：`len(entries)==len(raw)` 且 ID 与 raw 行号 1:1）。
  ...
  - Reference type：`legacy/regression-evidence` 164、`prose/example` 57、`output-directory` 9、`help/discovery` 9、`report-filename` 9、`identity/frontmatter` 6、`caller→callee` 4、`trigger/description` 4、`canonical-contract` 3、`workflow-prerequisite` 2、`rename-mapping` 2
...
| G003 | `assets/source/speclite/README.en.md:143` | EN | canonical source README | output-directory | `grill`, `speclite-implementation-readiness-grill-consistency-reviewer`, `{solutioning_artifacts}/implementation-readiness-report/grill-cons | speclite-implementation-readiness-grill-consistency-reviewer | active | none | keep |  |
...
| G006 | `assets/source/speclite/README.md:165` | ZH | canonical source README | output-directory | `grill`, `speclite-implementation-readiness-grill-consistency-reviewer`, `{solutioning_artifacts}/implementation-readiness-report/grill-cons | speclite-implementation-readiness-grill-consistency-reviewer | active | none | keep |  |
...
| G019 | `assets/source/speclite/core-skills/speclite-grilling/SKILL.en.md:20` | EN | speclite-grilling | caller→callee | `assets/source/speclite/core-skills/speclite-grilling/` | speclite-grilling | active | none | keep |  |
...
| G022 | `assets/source/speclite/core-skills/speclite-grilling/SKILL.md:20` | ZH | speclite-grilling | caller→callee | `assets/source/speclite/core-skills/speclite-grilling/` | speclite-grilling | active | none | keep | dirty-worktree（未提交改动） |
...
| G032 | `assets/source/speclite/sdlc-skills/3-solutioning/speclite-implementation-readiness-check/SKILL.en.md:18` | EN | speclite-implementation-readiness-check | report-filename | `{planning_artifacts}/ir-grill/`, `{solutioning_artifacts}/implementation-readiness-report/grill-consistency/implementation-readiness-report | {solutioning_artifacts}/implementation-readiness-report/grill-consistency | compatibility | 11.8 read-only legacy discovery clause (11.8 ledger: legacy-documentation) | keep (compatibility) | 同一行同时含新 output 路径与 legacy ˋir-grill/ˋ 只读发现条款 |
...
| G099 | `assets/source/speclite/sdlc-skills/module-help.csv:35` | shared | sdlc module metadata/help | help/discovery | `grill`, `Grill`, `speclite-implementation-readiness-grill-consistency-reviewer`, `{solutioning_artifacts}/implementation-readiness-report/g | speclite-implementation-readiness-grill-consistency-reviewer | active | none | keep | outputs 列 ˋir grill recordsˋ → 派生 artifactType ˋir-grill-recordsˋ |
...
| G110 | `docs/reference/skills/sdlc-workflows.md:71` | ZH | public docs | help/discovery | `grill`, `speclite-implementation-readiness-grill-consistency-reviewer`, `{solutioning_artifacts}/implementation-readiness-report/grill-cons | speclite-implementation-readiness-grill-consistency-reviewer | active | none | keep |  |
...
| G113 | `docs/reference/workflow-artifact-layout.md:170` | ZH | public docs | output-directory | `ir-grill/`, `speclite-implementation-readiness-grill-consistency-reviewer`, `{solutioning_artifacts}/implementation-readiness-report/grill- | speclite-implementation-readiness-grill-consistency-reviewer | active | none | keep |  |
...
| G176 | `test/fixtures/fresh-install-empty-project/expected/installed-state/phase-coverage-full.json:541` | shared | fresh-install fixture | legacy/regression-evidence | `ir-grill-records` | {planning_artifacts}/ir-grill/ (legacy path) | fixture | none | confirm | 安装投影快照（由 canonical 派生）；artifactType 名含旧 ˋir-grillˋ 词形（非路径形态，非 11.8 token） |
...
- `speclite-implementation-readiness-grill-consistency-reviewer` 与 `speclite-implementation-readiness-check`：module-help `preceded-by`（G099：IRG 前置 IR）+ reviewer SKILL:37 "可配合"（G065 ZH / G054 EN）——协作 / 前置关系，无显式 invocation。reviewer CHANGELOG:12 明确"移除对外部 grill skill 名称的显式依赖"，其 `grill-with-docs` 五处提及（G043, G046, G049, G059, G072, G087, G088）均为"内建协议来源"说明，**不是**对 `speclite-grill-with-docs` 的调用。

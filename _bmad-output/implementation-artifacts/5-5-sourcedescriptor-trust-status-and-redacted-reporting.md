...
- [ ] Task 1: 验证前置实现、工作树和只读边界（AC: 1-7）
  ...
  - [ ] 确认 Story 3.5 `CommandResult`/`ValidationIssue`、Story 3.2 manifest/index schema、Story 4.3 plan-before-write、Story 4.4 operation lock/safe write 和 Story 4.6 repair source policy anchors 是否真实存在；若不存在，不得创建私有 JSON shape、私有 trust model 或隐藏写入流程。
...
- `_bmad-output/implementation-artifacts/1-1` 到 `5-4` 当前是 ready-for-dev story context，不是完成后的源码证据。实现 Story 5.5 前必须重新确认前置 stories 是否已经由其它 agent 添加 actual implementation。

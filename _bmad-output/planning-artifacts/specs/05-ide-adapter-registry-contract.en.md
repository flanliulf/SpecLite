...
- `docs/specs/04-manifest-index-contract.en.md` owns installed manifest/index projections that consume adapter registry data.
...
These vocabularies must not be reused across layers.
...
If a user explicitly selects a target and that target is unsupported for the requested module set, install planning must produce a blocking error. If a target is optional or not selected, unsupported may be reported as warning, info, or known limitation according to `docs/specs/07-validation-issue-taxonomy.en.md`.

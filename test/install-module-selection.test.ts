...
describe("install official module selection orchestration", () => {
  ...
  it("selects default official modules after --yes and writes the confirmed install shape", async () => {
    ...
    try {
      ...
      expect(outcome.result.summary).toContain("Canonical package roots: core=13, sdlc=41, total=54.");
    ...
    try {
      ...
      const outcome = await runInstallCommand({
        ...
        configureProject: async (input) => {
          ...
          expect(input.prompt).toContain("Canonical package roots: core=13, sdlc=41, total=54.");
    ...
    try {
      ...
      const outcome = await runInstallCommand({
        ...
        confirmPrewriteInstallScope: async (input) => {
          ...
          expect(input.prompt).not.toContain("Canonical package roots: core=13, sdlc=41, total=54.");

...
describe("PRD validation report path contract", () => {
  ...
  it("keeps every active producer, consumer, help, contract and example surface classified", async () => {
    ...
    const supportReferenceAllowlist = new Map<string, Array<{ clause: string; occurrences: number }>>([
      ...
      ["docs/reference/skills/sdlc-workflows.md", [{ clause: "| `speclite-validate-prd` | Workflow | `VP` | `{planning_artifacts}/prd/prd-validate-report-{yyyy-MM-dd}.md` | 通过 shared whole/sharded resolver 校验 PRD；每次 invocation 只锁定一个 dated report，同日目标已存在时以 `artifact-path.prd-validation-report-exists` read-only block，不 overwrite / suffix，legacy report 不迁移。完整写入规约见 [`../workflow-artifact-layout.md`](../workflow-artifact-layout.md)。 |", occurrences: 1 }]],
      ["docs/reference/workflow-artifact-layout.md", [{ clause: "| `{planning_artifacts}/prd/prd-validate-report-{yyyy-MM-dd}.md` | `speclite-validate-prd` | 每次 invocation 只生成一次日期并锁定 exact path；private `scripts/prd-validation-report-operation.mjs` 以 exclusive create 写入。同日目标已存在时用 `artifact-path.prd-validation-report-exists` 阻断，人工处置为“保留并移走或删除既有报告后重新运行”；legacy report 原位可发现，不迁移。 |", occurrences: 1 }]],

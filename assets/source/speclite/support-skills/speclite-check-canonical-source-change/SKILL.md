...
[Core Capabilities（核心能力）]
    ...
    - **Strict mode**：通过 `--mode strict` 将 `D0` warning 升级为 error，供 CI 或 release gate 使用。
...
[Workflow（执行流程）]
    ...
    5. 若脚本输出 `governance.decisionRecordRequired: true`，使用 `speclite-canonical-source-governance-runner` 记录 D1/D2 的更新或跳过理由。
    6. 若涉及 Agent 定义包，补跑：
    ...
    8. 完成修复后重新运行 Step 3，并补跑 strict mode：

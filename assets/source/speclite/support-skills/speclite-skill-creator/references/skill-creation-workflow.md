...
需要保留外部 forge mirror 时，在 canonical 源定义完成并通过检查后，同步到 `/Users/fancyliu/Repos/skills-creator/forge/speclite/` 对应分区；不得把该 mirror 当成 SpecLite 项目的主源头。
...
- `name` 必须以 `speclite-` 开头，并与目录名一致。
- `metadata.version` 与 `metadata.author` 必填；`metadata.catalog` 固定使用 `speclite`，并与 SKILL.en.md 对齐。
- 不生成未登记的 `metadata.*` 字段。
...
如果在安装副本中执行，可使用对应 `speclite-skill-lint/scripts/check_skill_density.py`。
...
[Generation Metadata（生成信息）]
    本 Skill 由 speclite-skill-creator 自动生成。如需修改，必须同步更新 SKILL.md 与 SKILL.en.md，并同步 `<canonical-source-path>/` 与实际安装副本。
```
...
```text
[Generation Metadata]
    This Skill was generated by speclite-skill-creator. Update SKILL.md and SKILL.en.md together, and sync `<canonical-source-path>/` with installed copies.
```

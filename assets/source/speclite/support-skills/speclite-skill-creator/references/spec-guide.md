...
- `metadata.catalog`：可选；当 Skill 归入 catalog 目录时填写，值使用 kebab-case，并与 `assets/source/speclite/<group>/<skill-name>/` 路径及 SKILL.en.md mirror 保持一致。未归入 catalog 时省略。

不得擅自新增未登记的 `metadata.*` 字段；确需扩展时，必须同步更新 `speclite-skill-creator`、`speclite-skill-lint` 和 `skills-upgrade` 的字段契约。

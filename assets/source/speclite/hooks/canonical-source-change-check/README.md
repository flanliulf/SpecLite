...
`canonical-source-change-check` 是 SpecLite canonical source 维护时的 warning-only hook。它在 Claude Code / Codex 修改 `assets/source/speclite/` 后提醒运行 `speclite-canonical-source-governance-runner` 和 `speclite-check-canonical-source-change`，但不阻断会话。
...
- 输出 warning JSON，提醒 Agent 先执行 governance runner 分类影响面、记录 D1/D2 决策，再执行检查 Skill、刷新 fixtures/docs/packaging manifest 并运行验证命令。

...
`foundationPrerequisiteStatus` 与 `closureOwnerCheckStatus` 的语义判定由 `speclite-flow-gate` 基于项目自己的 foundation handoff manifest 生成；本 hook 只负责 fail-closed 消费这些机器字段。

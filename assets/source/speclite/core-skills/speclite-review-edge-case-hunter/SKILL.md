...
- **完整性复查**：回看每类边界，补充遗漏发现。
- **严格 JSON 输出**：只返回符合 schema 的 JSON array，并携带稳定 fingerprint seed 所需字段。
...
## Workflow（工作流）
    ...
    6. 输出 JSON array；每项只包含 `category`、`invariant`、`location`、`trigger_condition`、`actual_result`、`expected_result`、`guard_snippet`、`potential_consequence`。
...
## Notes（注意事项）
    ...
    - 不能给出具体 trigger condition 与错误 actual result 时不得输出 finding。

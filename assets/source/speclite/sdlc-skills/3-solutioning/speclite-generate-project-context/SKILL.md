...
description: "执行 SpecLite Generate Project Context workflow，生成供 AI agent 使用的项目上下文。用于用户要求 generate project context、创建 project-context.md、刷新项目上下文或运行上下文生成。核心能力：配置驱动激活、发现来源产物、按步骤编排、生成 `project-context.md` 并交接完成。"
...
[技能说明]
    Generate Project Context 是 Speclite 方案与架构准备工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。
...
[执行流程]
    1. 先完整阅读 `references/workflow-details.md`；该文件是从源入口转换后的权威工作流规约。随后按需读取 `references/workflow-details.md`、`references/steps/step-01-discover.md`、`references/steps/step-02-generate.md`、`references/steps/step-03-complete.md`。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-generate-project-context`。
    - `references/workflow-details.md` 和配套 reference 文件均为有效执行规约，不是背景资料。
    - 模板或示例骨架位于 `assets/project-context-template.md`。
    - 如工作流需要项目配置，必须读取目标项目运行时配置。
    - `config.toml.example` 仅作字段结构参考，不作为 runtime fallback。
    - 当前运行规约不得依赖旧运行目录、旧 YAML 配置或旧命令命名空间。
    - 输出文档末尾必须追加 `*本文档由 speclite-generate-project-context Skill 自动生成*` 标注。

...
- **不设最低问题数**：只报会实质阻塞实现的问题，无实质问题时报零；严禁为凑数而制造问题。
- **可验证性路由**：可由编译器/测试判定的属性——穷尽性/状态机全函数（totality）、determinism、replay 等价、幂等（idempotency）、资源无泄漏（no-leak）、类型/shape 一致性——**不得作为阻塞性散文问题**。如确有必要，仅作为一条 `verify-obligation` 标注（“应补一个断言 X 的测试”），交实现阶段的可执行门禁，而非要求修改设计文档。理由：散文无法被证明为“total”，只有代码 + 测试能。
- **新颖性要求**：不得仅因“上一轮为满足同类问题而新增的表述又存在一个未穷尽的组合格”就再次提出同类问题；除非能给出具体的新失败场景（具体输入/状态 → 错误结果），否则判为噪音不提。
- **元数据非阻塞**：provenance / round 号 / current-pointer / 轮次指针漂移属于可机械同步的记账问题，**永不作为阻塞项**；至多标注为一条待机械修正项。
...
只报会实质阻塞实现的问题；不设最低数量，无实质问题时报零，严禁为凑数提问。每个问题必须包含：具体的文件名、章节名、问题内容和改进建议。
...
在四桶分类前，先按 B0.5 的共同约束过滤：

- 命中**可验证性路由**的发现 → 桶 `verify-obligation`（不作阻塞 patch）。
- 命中**新颖性**排除（仅“又一未穷尽组合格”且无新失败场景）→ 桶 `dismiss`。
- 命中**元数据非阻塞**（provenance/round/pointer 漂移）→ 桶 `verify-obligation` 或 `dismiss`，绝不进 `patch`/`decision_needed`。
...
| `verify-obligation` | 可由编译器/测试判定的属性，或元数据机械同步 | totality/determinism/replay/幂等/no-leak/shape 一致性；provenance/round 漂移 |
| `defer` | 既有问题，非本次 Story 引入 | 前序 Epic 遗留的契约不一致 |
| `dismiss` | 噪音、误报、已处理，或仅“又一未穷尽组合格”无新失败场景 | 审查理解偏差、已在其他章节覆盖 |

...
- UX：`_speclite-output/planning-artifacts/ux-design-specification/index.md` 及同目录 shard；按需读取 UX direction、wireframe、design-system 输出。
...
- Optional workflow support：`speclite-check-implementation-readiness` 用于 readiness discovery 或报告复核。
...
若项目没有 `_speclite-output/planning-artifacts/`，使用：
...
```text
.specskills/docs/analysis/speclite-ir-grill-consistency-reviewer/
```

...
[Skill Overview]
    Domain Research is a Speclite 分析与项目发现 workflow Skill. It executes the migrated workflow in a target project using the configured communication language, document output language, and runtime paths.

    Source entry summary: Conduct domain and industry research. Use when the user says wants to do domain research for a topic or industry
...
[Workflow]
    1. Fully read `references/workflow-details.md`; it is the authoritative migrated workflow specification. Related references include `references/workflow-details.md`, `references/steps/step-01-init.md`, `references/steps/step-02-domain-analysis.md`, `references/steps/step-03-competitive-landscape.md`, `references/steps/step-04-regulatory-focus.md`, `references/steps/step-05-technical-trends.md`, and other reference files.
    ...
    5. Before exit, run the quality checks, resolve `workflow.on_complete`, and append `*This document was generated by the speclite-domain-research Skill*` to generated documents.

[Notes]
    - The directory name and YAML `name` must remain `speclite-domain-research`.

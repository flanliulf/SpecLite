...
description: "执行 SpecLite Domain Research workflow，调研业务领域并生成规划上下文。用于用户要求 domain research、speclite domain research、创建领域研究或运行领域研究。核心能力：配置驱动激活、发现来源产物、按步骤编排、生成输出并交接完成。"
...
[技能说明]
    Domain Research 是 Speclite 分析与项目发现工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Conduct domain and industry research. Use when the user says wants to do domain research for a topic or industry
...
[执行流程]
    1. 先完整阅读 `references/workflow-details.md`；该文件是从源入口转换后的权威工作流规约。随后按需读取 `references/workflow-details.md`、`references/steps/step-01-init.md`、`references/steps/step-02-domain-analysis.md`、`references/steps/step-03-competitive-landscape.md`、`references/steps/step-04-regulatory-focus.md`、`references/steps/step-05-technical-trends.md` 等 reference 文件。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-domain-research`。
    ...
    - 输出文档末尾必须追加 `*本文档由 speclite-domain-research Skill 自动生成*` 标注。

...
description: "执行 SpecLite Technical Research workflow，研究技术选型、架构约束与实现风险。用于用户要求 technical research、speclite technical research、创建技术研究或运行技术研究。核心能力：配置驱动激活、发现来源产物、按步骤编排、生成输出并交接完成。"
...
[技能说明]
    Technical Research 是 Speclite 分析与项目发现工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Conduct technical research on technologies and architecture. Use when the user says they would like to do or produce a technical research report
...
[执行流程]
    1. 先完整阅读 `references/workflow-details.md`；该文件是从源入口转换后的权威工作流规约。随后按需读取 `references/workflow-details.md`、`references/steps/step-01-init.md`、`references/steps/step-02-technical-overview.md`、`references/steps/step-03-integration-patterns.md`、`references/steps/step-04-architectural-patterns.md`、`references/steps/step-05-implementation-research.md` 等 reference 文件。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-technical-research`。
    - `references/workflow-details.md` 和配套 reference 文件均为有效执行规约，不是背景资料。
    - 模板或示例骨架位于 `assets/research.template.md`。
    - 研究输出默认写入 `{analysis_artifacts}/research/`；可读取 `{project_knowledge}` 作为上下文，但不得把 research 输出写入 `{project_knowledge}` 或 `docs/`。
    - 如工作流需要项目配置，必须读取目标项目运行时配置与 resolved artifact roots。
    - `config.toml.example` 仅作字段结构参考，不作为 runtime fallback。
    - 当前运行规约不得依赖旧运行目录、旧 YAML 配置或旧命令命名空间。
    - 输出文档末尾必须追加 `*本文档由 speclite-technical-research Skill 自动生成*` 标注。

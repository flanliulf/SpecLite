...
[Skill Overview]
    Technical Research is a Speclite 分析与项目发现 workflow Skill. It executes the migrated workflow in a target project using the configured communication language, document output language, and runtime paths.

    Source entry summary: Conduct technical research on technologies and architecture. Use when the user says they would like to do or produce a technical research report
...
[Workflow]
    1. Fully read `references/workflow-details.md`; it is the authoritative migrated workflow specification. Related references include `references/workflow-details.md`, `references/steps/step-01-init.md`, `references/steps/step-02-technical-overview.md`, `references/steps/step-03-integration-patterns.md`, `references/steps/step-04-architectural-patterns.md`, `references/steps/step-05-implementation-research.md`, and other reference files.
    ...
    5. Before exit, run the quality checks, resolve `workflow.on_complete`, and append `*This document was generated by the speclite-technical-research Skill*` to generated documents.

[Notes]
    - The directory name and YAML `name` must remain `speclite-technical-research`.
    - `references/workflow-details.md` and related references are executable instructions, not background reading.
    - Research outputs default to `{analysis_artifacts}/research/` resolved by `speclite resolve artifact-roots`; the workflow may read `{project_knowledge}` as context, but must not write research outputs to `{project_knowledge}` or `docs/`.

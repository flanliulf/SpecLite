...
description: "执行 SpecLite Market Research workflow，研究市场、竞品与客户信号。用于用户要求 market research、speclite market research、创建市场研究或运行市场研究。核心能力：配置驱动激活、发现来源产物、按步骤编排、生成输出并交接完成。"
...
[技能说明]
    Market Research 是 Speclite 分析与项目发现工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Conduct market research on competition and customers. Use when the user says they need market research
...
[执行流程]
    1. 先完整阅读 `references/workflow-details.md`；该文件是从源入口转换后的权威工作流规约。随后按需读取 `references/workflow-details.md`、`references/steps/step-01-init.md`、`references/steps/step-02-customer-behavior.md`、`references/steps/step-03-customer-pain-points.md`、`references/steps/step-04-customer-decisions.md`、`references/steps/step-05-competitive-analysis.md` 等 reference 文件。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-market-research`。
    ...
    - 输出文档末尾必须追加 `*本文档由 speclite-market-research Skill 自动生成*` 标注。

...
[Skill Overview]
    Document Project is a Speclite 分析与项目发现 workflow Skill. It executes the migrated workflow in a target project using the configured communication language, document output language, and runtime paths.

    Source entry summary: Document brownfield projects for AI context. Use when the user says "document this project" or "generate project docs"
...
[Workflow]
    1. Fully read `references/workflow-details.md`; it is the authoritative migrated workflow specification. Related references include `references/workflow-details.md`, `references/checklist.md`, `references/instructions.md`, `references/workflows/deep-dive-instructions.md`, `references/workflows/deep-dive-workflow.md`, `references/workflows/full-scan-instructions.md`, and other reference files.
    ...
    5. Before exit, run the quality checks, resolve `workflow.on_complete`, and append `*This document was generated by the speclite-document-project Skill*` to generated documents.

...
description: "执行 SpecLite Product Brief workflow，生成或更新产品愿景与范围简报。用于用户要求 product brief、创建产品简报、更新产品简报或运行产品简报流程。核心能力：配置驱动激活、发现来源产物、按步骤编排、生成输出并交接完成。"
...
[技能说明]
    Product Brief 是 Speclite 分析与项目发现工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。
...
[执行流程]
    1. 先完整阅读 `references/workflow-details.md`；该文件是从源入口转换后的权威工作流规约。随后按需读取 `references/workflow-details.md`、`references/agents/artifact-analyzer.md`、`references/agents/opportunity-reviewer.md`、`references/agents/skeptic-reviewer.md`、`references/agents/web-researcher.md`、`references/prompts/contextual-discovery.md` 等 reference 文件。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-product-brief`。

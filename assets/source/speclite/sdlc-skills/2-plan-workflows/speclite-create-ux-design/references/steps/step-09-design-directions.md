...
- When 'A' selected: Invoke the `speclite-advanced-elicitation` skill
- When 'P' selected: Invoke the `speclite-party-mode` skill
...
I'm creating a comprehensive HTML design direction showcase at `{actualDesignDirectionsPath}` (canonical default: `{planning_artifacts}/ux/ux-design-directions.html`)
...
- Invoke the `speclite-advanced-elicitation` skill with the current design direction content
...
- Invoke the `speclite-party-mode` skill with the current design direction
...
- Write the showcase to `{actualDesignDirectionsPath}`. Existing legacy siblings stay in place; only a missing sibling uses the canonical path. For a canonical create, place the exact generated bytes in `{source-file}` and run `node "{skill-root}/scripts/ux-artifact-operation.mjs" create-file --project-root "{project-root}" --planning-root "{planning_artifacts}" --target "{actualDesignDirectionsPath}" --source "{source-file}"`. Require exit `0`, exactly one JSON object, `ok: true`, the exact target and `operation: "create-file"`; non-zero, invalid JSON or `ok !== true` HALTs without append/frontmatter/progress advancement. The operation performs commit-time real UX owner / nearest-existing-ancestor revalidation and the immediate exclusive create. For local HTML `href` / `src`, ignore explicit external schemes and literal fragment/query-only values, but fail closed on any other raw value containing `&` before stripping or decoding; then resolve screenshots and auxiliary assets relative to the selected artifact's containing directory, percent-decode exactly once and reject malformed, encoded-separator, traversal, unreadable or symlink-escape targets before mutation.

...
[技能说明]
    Create Ux Design 是 Speclite 规划与产品设计工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。
...
[核心能力]
    - **UX 路径治理**：新产物统一写入 real `{planning_artifacts}/ux/` physical owner 内的三个精确文件；exclusive create 与 `design-system/` on-demand mkdir 必须由 private `{skill-root}/scripts/ux-artifact-operation.mjs` 在 commit-time physical-owner / nearest-existing-ancestor revalidation 后立即执行，不向调用方返回 approval 后再自行写入。`create-file` 使用 `node "{skill-root}/scripts/ux-artifact-operation.mjs" create-file --project-root "{project-root}" --planning-root "{planning_artifacts}" --target "{target}" --source "{source-file}"`；`create-directory` 使用 `node "{skill-root}/scripts/ux-artifact-operation.mjs" create-directory --project-root "{project-root}" --planning-root "{planning_artifacts}" --target "{target}"`。stdout 必须是恰一个 JSON object；non-zero、invalid JSON 或 `ok !== true` 必须 HALT 且不得推进 frontmatter/progress/append target。该 script 是 Skill-private binding，不是 public `speclite` CLI。Existing legacy UX artifacts 仅在 real Planning owner 内只读发现并原位继续。本地 Markdown duplicate definition 采用 first-definition-wins；local-ish HTML `href` / `src` raw value 含 `&` 时 fail closed。

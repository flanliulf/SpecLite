...
[Overview（技能说明）]
    面向开源 Node.js 项目的 npm 发布执行 Skill。它把发布前事实审计、真实 tarball smoke、官方 registry 登录、人工确认、正式 publish、发布后验证和故障恢复串成可复用流程，避免版本复用、镜像 registry 凭据混淆、`.bin` 入口失效和 read-side propagation delay 误判。
...
[Workflow（执行流程）]
    ...
    Step 3：执行发布前门禁
        按 reference 的 Prepublish Gates 顺序验证 official registry 登录、版本未占用、metadata、scripts、构建、测试和真实 tarball smoke。任何门禁失败都先修复根因或要求用户决策，不降级需求。

    Step 4：发布与异常处理
        仅在用户确认后运行 publish。scoped public package 使用 `--access public`；unscoped package 不强行添加 scoped-only 语义。OTP、password、token 只在终端交互输入。

...
description: "执行 SpecLite Sprint Status workflow，读取 sprint 状态并推荐下一步动作。用于用户要求 sprint status、check sprint status、查看 sprint、下一步 story 或状态汇总。核心能力：解析 `sprint-status.yaml`、识别阻塞与就绪项、输出推荐路由。"
...
[技能说明]
    Sprint Status 是 Speclite 实施与交付推进工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Summarize sprint status and surface risks. Use when the user says "check sprint status" or "show sprint status"
...
[核心能力]
    ...
    - **Flow Gate 推荐**：在推荐 `speclite-dev-story` 前检查 ready-for-dev Story 的 story-kickoff gate，缺失、失败、过期、legacy v1、缺少 handoff contract version、缺少 foundation prerequisite metadata 或 closure owner metadata 时优先推荐 `speclite-flow-gate`。
...
[执行流程]
    ...
    4. 推荐进入 `speclite-dev-story` 前，按 `references/workflow-details.md` 检查目标 Story 的 story-kickoff gate 是否为 v2 report，`handoffContractVersion` 是否匹配，result 是否为 `PASS` 或 `PASS_EQUIVALENT`，且 `foundationPrerequisiteStatus`、`closureOwnerCheckStatus` 均允许继续。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-sprint-status`。
    ...
    - 输出文档末尾必须追加 `*本文档由 speclite-sprint-status Skill 自动生成*` 标注。

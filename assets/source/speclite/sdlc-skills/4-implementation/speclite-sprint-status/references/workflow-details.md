# Sprint Status Workflow Details
...
Run `/speclite:bmm:workflows:sprint-planning` to generate it, then rerun sprint-status.</output>
...
- IF any ready-for-dev story's latest kickoff gate is not `schemaVersion=speclite.flow-gate-report.v2`, or is missing `handoffContractVersion=speclite.story-kickoff-handoff.v1`: warn "story-kickoff gate uses a legacy or incomplete handoff schema; rerun speclite-flow-gate before dev-story"
...
<step n="3" goal="Select next action recommendation">
  ...
  3. Else if any story status == ready-for-dev AND the first ready story has no current kickoff gate with `schemaVersion=speclite.flow-gate-report.v2`, `handoffContractVersion=speclite.story-kickoff-handoff.v1`, result `PASS` or `PASS_EQUIVALENT`, and allowing `foundationPrerequisiteStatus` / `closureOwnerCheckStatus` → recommend `flow-gate` for that story
...
**Next Recommendation:** /speclite:bmm:workflows:{{next_workflow_id}} ({{next_story_id}})

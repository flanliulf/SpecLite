...
description: "执行 SpecLite Correct Course workflow，处理 sprint 期间的重大范围或方向调整。用于用户要求 correct course、sprint change、变更路线、提出 sprint change proposal 或修正计划。核心能力：配置驱动激活、发现上下文、按步骤分析影响、输出变更建议。"
...
[技能说明]
    Correct Course 是 Speclite 实施与交付推进工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Manage significant changes during sprint execution. Use when the user says "correct course" or "propose sprint change"

[核心能力]
    - **UX 输入路径治理**：从 `{planning_artifacts}/ux/ux-design-specification.md` 或只读 legacy fallback 加载 UX，并记录 Planning root 的 `resolvedRoot`、`resolutionMode`、`actualConsumedPath`，禁止迁移或路径逃逸。
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-correct-course`。
    ...
    - 输出文档末尾必须追加 `*本文档由 speclite-correct-course Skill 自动生成*` 标注。

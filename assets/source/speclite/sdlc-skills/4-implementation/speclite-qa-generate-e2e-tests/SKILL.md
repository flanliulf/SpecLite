...
description: "执行 SpecLite QA Generate E2E Tests workflow，为既有功能生成端到端自动化测试。用于用户要求 qa generate e2e tests、create e2e tests、生成 E2E 测试或 QA 自动化。核心能力：配置驱动激活、发现目标功能、按步骤编排、生成测试输出并交接完成。"
...
[技能说明]
    QA Generate E2e Tests 是 Speclite 实施与交付推进工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Generate end to end automated tests for existing features. Use when the user says "create qa automated tests for [feature]"
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-qa-generate-e2e-tests`。

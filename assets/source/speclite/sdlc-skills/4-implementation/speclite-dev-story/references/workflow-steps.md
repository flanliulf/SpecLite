...
- 读取 gate report YAML frontmatter metadata，确认 `schemaVersion=speclite.flow-gate-report.v2`、`handoffContractVersion=speclite.story-kickoff-handoff.v1`、`mode=story-kickoff` 且结果为 `PASS` 或 `PASS_EQUIVALENT`。
...
- 不得用 Markdown prose、人工摘要或历史记忆替代上述 frontmatter 字段；closure owner 是否正确以 `speclite-flow-gate` 读取项目 foundation handoff source index 或 Story/Epic 显式 references 后写入的 metadata 为准。
...
- 若适用，确认部署就绪
- 运行 `code-review-01-reviewer` 工作流进行同行评审
- 可选：若已安装 Test Architect 模块，运行对应的自动化测试生成流程来扩充护栏测试

### 10.9 / 10.10 / 10.11
- 10.9 输出："提示：为获得最佳效果，请使用与本次实现不同的 LLM 来运行 `code-review-01-reviewer`。"

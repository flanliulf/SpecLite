...
[Skill Overview]
    Retrospective is a Speclite 实施与交付推进 workflow Skill. It executes the migrated workflow in a target project using the configured communication language, document output language, and runtime paths.
...
[Workflow]
    ...
    5. Before exit, run the quality checks, resolve `workflow.on_complete`, and append `*This document was generated by the speclite-retrospective Skill*` to generated documents.

[Notes]
    - The directory name and YAML `name` must remain `speclite-retrospective`.

...
[技能说明]
    Retrospective 是 Speclite 实施与交付推进工作流 Skill，用于在目标项目中按配置语言、输出语言和工作流步骤完成对应制品或交付动作。

    源入口说明：Post-epic review to extract lessons and assess success. Use when the user says "run a retrospective" or "lets retro the epic [epic]"
...
[注意事项]
    - 名称、目录与 YAML `name` 字段保持 kebab-case 一致：`speclite-retrospective`。
    ...
    - 输出文档末尾必须追加 `*本文档由 speclite-retrospective Skill 自动生成*` 标注。

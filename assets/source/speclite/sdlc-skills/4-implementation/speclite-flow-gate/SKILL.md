...
[Core Capabilities（核心能力）]
    ...
    - **Foundation handoff 校验**：在 `story-kickoff` 中读取项目提供的 foundation handoff source index、downstream prerequisite 与 future closure source，固化 v2 report 的 `handoffContractVersion`、`foundationPrerequisiteStatus` 与 `closureOwnerCheckStatus`，确保后续 Story 不引用错误 closure owner。
...
[Workflow（执行流程）]
    ...
    4. 按 `Contract -> Functional -> Evidence -> Guidance -> Foundation Handoff` 顺序做门控判断，并生成 `assets/report-template.md` 规定的 v2 报告。
...
[Notes（注意事项）]
    ...
    - 报告开头的 YAML frontmatter 是 downstream hook、dev-story、CR 和 finalizer 的 machine-readable source；不得要求消费者解析 Markdown prose 判断 gate result、foundation prerequisite 或 closure owner；legacy v1 report 必须重新生成 v2。

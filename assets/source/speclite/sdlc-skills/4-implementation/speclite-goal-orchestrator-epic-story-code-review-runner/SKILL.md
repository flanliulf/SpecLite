...
- 只有 `PASS` 或 `PASS_EQUIVALENT` 允许进入 Story 开发；`FAIL_CONTRACT`、`FAIL_FUNCTION`、`FAIL_EVIDENCE`、`DECISION_NEEDED` 或 metadata 缺失/不匹配/过期时必须停止开发。
- 如果门禁未通过，根据 gate report 的 recommended next action 记录下一步；除非用户明确授权，不得擅自修订当前 Story/Epic 之外的文件。
- 将 gate report 路径、result、是否 `PASS_EQUIVALENT`、继续/停止决策写入 `PLAN.md`、`EXPERIMENTS.md`、`EXPERIMENT_NOTES.md`。
...
- `story-kickoff` Flow Gate result 为 `PASS` 或 `PASS_EQUIVALENT`，且 gate report metadata 匹配当前 Story。
- 开发完成。
- 最新 CR reviewer 通过。
- 最新 CR evaluator 通过。
...
- Epic 范围内每个目标 Story 均有当前 Story 匹配的 `story-kickoff` Flow Gate report，且 result 为 `PASS` 或 `PASS_EQUIVALENT`。

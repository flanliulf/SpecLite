#!/usr/bin/env python3
"""
Resolve SpecLite's central config using four-layer TOML merge.

Reads from four layers (highest priority last):
    1. {project-root}/_speclite/config.toml              (installer-owned team)
    2. {project-root}/_speclite/config.user.toml         (installer-owned user)
    3. {project-root}/_speclite/custom/config.toml       (human-authored team, committed)
    4. {project-root}/_speclite/custom/config.user.toml  (human-authored user, gitignored)
...
no virtualenv — plain `python3` is sufficient.
  ...
  python3 resolve_config.py --project-root ... --key core
    python3 resolve_config.py --project-root ... --key modules.sdlc
...
def main():
    parser = argparse.ArgumentParser(
        description="Resolve SpecLite central config using four-layer TOML merge.",
    )
    parser.add_argument(
        "--project-root", "-p", required=True,
        help="Absolute path to the project root (contains _speclite/)",
    )
    parser.add_argument(
    ...
    speclite_dir = project_root / "_speclite"

    base_team = load_toml(speclite_dir / "config.toml", required=True)
    base_user = load_toml(speclite_dir / "config.user.toml")
    custom_team = load_toml(speclite_dir / "custom" / "config.toml")
    custom_user = load_toml(speclite_dir / "custom" / "config.user.toml")

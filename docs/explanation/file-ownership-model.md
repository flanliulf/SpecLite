...
四类 ownership 在 `update`、`update --repair`、`uninstall` 中的行为、示例与 reason code 以 [`../reference/ownership-matrix.md`](../reference/ownership-matrix.md) 为唯一源定义。概括地说：installer-owned 可被计划写入、修复或移除；human-owned 与 workflow-owned 一律 `skip` / `preserve` / `manual-action`，不产生 conflict；unknown 一律 `conflict` 并阻断自动写入。

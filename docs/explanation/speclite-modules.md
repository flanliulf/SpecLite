...
| `solutioning_artifacts` | `{output_folder}/3-solutioning-artifacts` | 存放 Architecture、Specs、implementation readiness 等 solutioning 产物。 |
| `implementation_artifacts` | `{output_folder}/4-implementation-artifacts` | 存放 Sprint、Story、Flow Gate、Review、QA、Retrospective 与 Quick Dev 产物。 |
| `devops_artifacts` | `{output_folder}/5-devops-artifacts` | 存放 CI/CD、deployment 与 npm release 产物。 |
| `project_knowledge` | `{output_folder}/project-knowledge-base` | 存放 Brownfield baseline、长期项目知识和 TSD 等 Project Knowledge 产物。 |

Module 还声明安装时要创建的目录，包括 `0-brainstorming-artifacts`、`1-analysis-artifacts/{product-brief,prfaq,research}`、`2-planning-artifacts/{epics,prd,ux}`、`3-solutioning-artifacts/{architecture,specs,implementation-readiness-report/grill-consistency}`、`4-implementation-artifacts/{stories,code-reviews,story-reviews,flow-gates,cr-rules,retrospectives}`、`5-devops-artifacts/{ci-cd,deployments,npm-releases}` 和 `project-knowledge-base/{brownfield,tsd}` 等。Public Documentation（公开文档）仍使用 `docs/` 信息架构，不等同于 fresh `project_knowledge` 默认目录。七个 plane 的 config key、默认值与用途以 [`../reference/workflow-artifact-layout.md`](../reference/workflow-artifact-layout.md) 为唯一源定义。

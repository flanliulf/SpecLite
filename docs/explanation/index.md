...
| [`ide-discovery-metadata.md`](ide-discovery-metadata.md) | self-contained Skill entry、installed indexes 与 IDE-specific runtime config 的边界。 |
...
| [`flow-gate-handoff-pitfalls.md`](flow-gate-handoff-pitfalls.md) | Flow Gate handoff 三层结构中各角色的位置与常见误解。 |
| [`codex-claude-code-sessions.md`](codex-claude-code-sessions.md) | Research Note：Codex 与 Claude Code 本机会话定位、恢复与 deep link 的调研记录。 |

...
当前 `SpecLite SDLC Module` 在 `module.yaml` 中声明 7 个 Agent：Alice（Business Analyst）、Taylor（Technical Writer）、Sarah（Open Source Docs Steward）、Paul（Product Manager）、Uma（UX Designer）、Adam（System Architect）和 David（Senior Software Engineer）。每个 Agent 的 ID、package 目录、catalog phase、菜单码、职责与 runtime 投影角色以 [`../reference/skills/agent-roster.md`](../reference/skills/agent-roster.md) 为唯一源定义。

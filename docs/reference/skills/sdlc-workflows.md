...
| 当前 package roots | 50 个带 `SKILL.md` 的目录 |
...
当前 SDLC Module 在 `module.yaml` 中声明 7 个 Agent。Agent 负责 role activation 和 workflow 分发，不是普通执行 workflow。完整名录（ID、persona、目录、catalog phase、菜单码、职责）以 [`agent-roster.md`](agent-roster.md) 为唯一源定义；下面各阶段表只保留 Agent 的 catalog 行。
...
| `speclite-create-ux-design` | Workflow | `CU` | `{planning_artifacts}/ux` | 创建 `ux-design-specification.md` 与条件 HTML 视觉产物；exclusive create 由 Skill-private `scripts/ux-artifact-operation.mjs` 执行，失败时 HALT 且不推进状态。完整写入规约见 [`../workflow-artifact-layout.md`](../workflow-artifact-layout.md)。 |
| `speclite-edit-prd` | Workflow | `EP` | `{planning_artifacts}` | 修订既有 PRD。 |
| `speclite-validate-prd` | Workflow | `VP` | `{planning_artifacts}/prd/prd-validate-report-{yyyy-MM-dd}.md` | 通过 shared whole/sharded resolver 校验 PRD；每次 invocation 只锁定一个 dated report，同日目标已存在时以 `artifact-path.prd-validation-report-exists` read-only block，不 overwrite / suffix，legacy report 不迁移。完整写入规约见 [`../workflow-artifact-layout.md`](../workflow-artifact-layout.md)。 |
...
| `speclite-retrospective` | Workflow | `ER` | `{implementation_artifacts}/retrospectives` | 执行 Epic 或 sprint 复盘。 |
